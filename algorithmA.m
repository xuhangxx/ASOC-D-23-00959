...
max_mgot=0;
num_m=zeros(1,100);
% going through all possible X
num_fenpei=power(n,m);
lenmax=0;
list=zeros(num_fenpei,m);
len=zeros(1,m);
% for i=1:num_fenpei  %list保存目标被分配所有的可能性
%     city=zeros(1,m);
%     m_list=m;
%     pl=num_serving_T(m_list);
%     while flag_m(m_list)==0
%         m_list=m_list-1;
%         pl=num_serving_T(m_list);
%     end
%     city(1,m_list)=list_serving_codeold{m_list,1}(1,mod(i-1,pl)+1);
%     for t=1:(m_list-1)
%         if flag_m(m_list-t)==0
%             continue;
%         end
%         pn=pl*num_serving_T(m_list-t);
%         city(1,m_list-t)=list_serving_codeold{m_list-t,1}(1,ceil((mod(i-1,pn)+1)/pl));
%         pl=pn;
%     end
%     [len(i),sumt,sumfuel,flag,final,num_mi]=func1(m,city,RTOM,Mu,n,TOT,VOC,COEC,COET,final_pre,com_parameter);
%     list(i,:)=city;
%     if(flag~=0)
%         if(lenmax<len(i))
%             lenmax=len(i);
...
%         end
    ...
    if(number_iter>1)
        ...
        cc=len(number_iter-1)-len(number_iter);
        if(cc==0)
...
toc;
ttime=toc-tic;
eval(['save(''800a_result',num2str(iter),'.mat'');'])
end
...
for i=1:length(lenzq)
    if(flagzq(i)~=0)
        fitness(i,1)=(1-(maxlen-lenzq(i))/(maxlen-minlen+0.001));
    end
end
